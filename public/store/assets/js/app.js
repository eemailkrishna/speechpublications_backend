...
(function () {
  ...
  function addActiveClass(element) {
    ...
    if (current === "") {
      ...
      // For other url
      const hrefPath = (element.getAttribute('href') || '').split('?')[0].replace(/^[a-z]+:\/\/[^\/]+/i, '').replace(/^\/|\/$/g, '');
      // Exact path match only (no loose substring match, otherwise /admin-news would
      // also match /admin-news-author and /admin-news-category and open all menus,
      // and /admin-news-author would also match /admin-news-author/create)
      const isMatch = hrefPath && current === hrefPath;
      if (isMatch) {
        const elParents = getParents(element, '.nav-item');         // Getting parents of the 'element' with a class '.nav-item'
        elParents[elParents.length - 1].classList.add('active');    // Adding class 'active' to the outer(direct) '.nav-item'
  ...
  const current = location.pathname.split("?")[0].replace(/^\/|\/$/g, '');
